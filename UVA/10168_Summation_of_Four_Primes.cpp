...
void solve(l n){
    ...
    if((n&1)){
        ...
        if((n>>1)&1) {cnt1 = 2; cnt2 = 2;;n-=4;flag = false;}
        else n = (n>>1);
    }
    l res1 = 0, res2 = 0;
    for(l i = 0; i < sz && primes[i] <= (n>>1); i++){
        if(!(marked[(n-primes[i])])){
            res1 = primes[i]; res2 = n - primes[i];
            break;
        }
    }
    if(res1==0 || res2==0) {cout << "Impossible." << endl;return;}
    if(flag) cout << res1 << " " << res2 << " " << res1 << " " << res2 << endl;
    else cout << cnt1 << " " << cnt2 << " " << res1 << " " << res2 << endl;
