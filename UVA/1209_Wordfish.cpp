...
void solve(string s){
    l sz = s.size(); l x = 10; l mi = 30;
    string p = s;
    string result = "";
    l res = INT_MAX;
    for(l i = 0; i < sz-1; i++){
        res = min(res, (abs(p[i]-p[i+1])));
        result = p;
        mi = res;
    }
    ...
    while(x--){
        if(next_permutation(s.begin(), s.end())){
            ...
            //cout << res << endl;
            if(res > mi){ // greater than because we have to maximize min abs dis of consequtive letter
                // if(res == mi) result = min(result, s);
                // else result = s;
                result = s;
                mi = res;
            }
